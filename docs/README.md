# Documentation

This directory contains supporting documentation for Attexa Arc Harness.

Start with:

- [Project README](../README.md)
- [Current project status](../STATUS.md)
- [Architecture](../ARCHITECTURE.md)
- [Project Origin and Stewardship](ORIGIN-AND-STEWARDSHIP.md)
- [Roadmap](../ROADMAP.md)
- [Public status and milestone process](STATUS-UPDATES.md)
- [Milestone records](milestones/README.md)
- [Public import provenance](provenance/README.md)
- [Building Arc with Arc](BUILDING-IN-PUBLIC.md)
- [Arc Practice](practice/README.md)
- [Arc Practitioner Charter](practice/ARC-PRACTITIONER-CHARTER.md)
- [Continuity Handoff Test](practice/CONTINUITY-HANDOFF-TEST.md)
- [The Work Should Remain thesis series](thesis/README.md)
- [RFC process](../rfcs/README.md)
- [Contributing](../CONTRIBUTING.md)
- [Security](../SECURITY.md)

As the reference implementation is imported, this directory will expand to include installation, operations, recovery, provider integration, conformance, threat-model, and evidence documentation.
